...
function getRecommendedJobs($conn, $user_id, $limit = 10, $min_match_score = 40)
{
    ...
    $recommended = [];
    if (!$jobs_query) {
        error_log('Freelance Matching Online recommendations query failed: ' . mysqli_error($conn));
        return [];
    }
...
function getNearbyFreelancers($conn, $employer_user_id, $limit = 10, $min_match_score = 40, $radius_km = 30)
{
    ...
    if (!$freelancers_query) {
        error_log('Freelance Matching Online nearby freelancers query failed: ' . mysqli_error($conn));
        return [];
    }
